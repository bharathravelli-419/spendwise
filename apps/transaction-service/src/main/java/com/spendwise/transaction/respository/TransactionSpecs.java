...
public class TransactionSpecs {
    ...
    public static Specification<Transaction> occurredBetween(OffsetDateTime from, OffsetDateTime to){
        return ((root, query, criteriaBuilder) -> {
            ...
            if(from != null)
                return criteriaBuilder.greaterThanOrEqualTo(root.get("occurredAt"), from);
            if(to != null)
                return criteriaBuilder.lessThanOrEqualTo(root.get("occurredAt"),to);
            return null;
        });
    }
    public static Specification<Transaction> amountAtLeast(BigDecimal min){
        return min== null ? null : ((root, query, criteriaBuilder) -> criteriaBuilder.greaterThanOrEqualTo(root.get("amount"), min));
    }
}
